  ...
  if (!skipCertbot && domainsToCertify.length > 0) {
    ...
    setTimeout(async () => {
      ...
      for (const rootDomain of domainsToCertify) {
        const wwwDomain = 'www.' + rootDomain;
        try {
          console.log(`Checking DNS for ${rootDomain} and ${wwwDomain} before requesting SSL...`);
          ...
          console.log(`DNS verified. Requesting SSL certificate for ${rootDomain} and ${wwwDomain}...`);
          execSync(`sudo certbot --nginx --cert-name ${rootDomain} -d ${rootDomain} -d ${wwwDomain} --non-interactive --agree-tos --email asis@cloudrik.com --redirect --expand`);
          needsReload = true;
