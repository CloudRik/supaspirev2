...
[ -z "$REPO_URL" ] && { echo "Please provide GitHub repo URL"; exit 1; }
[ -z "$PORT" ] && PORT=3000
...
try:
    data = json.load(open('/home/ubuntu/backend/env_vars.json'))
    vs = data.get(sys.argv[1], {})
    print(vs.get('GITHUB_TOKEN', ''))
except: pass
" "$PROJECT_NAME" 2>/dev/null || true)
fi

if [ -z "$GITHUB_TOKEN" ] && [ -f "/home/ubuntu/backend/github_token.json" ]; then
  GITHUB_TOKEN=$(python3 -c "
import json
try:
    data = json.load(open('/home/ubuntu/backend/github_token.json'))
...
if [ -n "$GITHUB_TOKEN" ]; then
  if [[ "$REPO_URL" =~ ^https://github.com/ ]]; then
    STRIPPED_URL="${REPO_URL#https://}"
    CLONE_URL="https://${GITHUB_TOKEN}@${STRIPPED_URL}"
    echo "Private repository token detected — authenticating clone..."
  fi
fi
