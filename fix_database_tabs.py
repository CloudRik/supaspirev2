...
# 1. Let's add the activeDbSubTab state
# We can find: const [activeTab, setActiveTab] = useState<string>("Overview");
# And append right after it: const [activeDbSubTab, setActiveDbSubTab] = useState<string>("Schema Visualizer");
active_tab_str = 'const [activeTab, setActiveTab] = useState<string>("Overview");'
active_tab_idx = content.find(active_tab_str)
if active_tab_idx != -1:
    insert_pos = active_tab_idx + len(active_tab_str)
    content = content[:insert_pos] + '\n  const [activeDbSubTab, setActiveDbSubTab] = useState<string>("Schema Visualizer");' + content[insert_pos:]
    print("Added activeDbSubTab state hook successfully!")

# 2. Now let's replace the whole {activeTab === "Database" && ( ... )} section.
...
print(f"Replacing database block from index {db_tab_start_idx} to {db_tab_end_line_idx}")

new_database_block = '''{activeTab === "Database" && (
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden grid grid-cols-5 min-h-[560px]">
              
              {/* Database Sub-Sidebar */}
              <div className="col-span-1 border-r border-slate-200 bg-slate-50/50 p-4 flex flex-col justify-between">
                <div className="space-y-5">
                  
                  {/* Category: DATABASE MANAGEMENT */}
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block px-2 mb-2 select-none">
                      Database Management
                    </span>
                    <div className="space-y-0.5">
                      {[
                        { id: "Schema Visualizer", label: "Schema Visualizer" },
                        { id: "Tables", label: "Tables" },
                        { id: "Extensions", label: "Extensions" },
                      ].map((sub) => (
                        <button
                          key={sub.id}
                          onClick={() => setActiveDbSubTab(sub.id)}
                          className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-semibold block transition-colors ${
                            activeDbSubTab === sub.id
                  ...
                  <div>
                    ...
                    <div className="space-y-0.5">
                      {[
                        ...
                        { id: "Database Webhooks", label: "Database Webhooks" },
                      ].map((sub) => (
                        <button
                          key={sub.id}
                          onClick={() => setActiveDbSubTab(sub.id)}
              ...
              {/* Subtabs Content Area (Right side — Col-span 4) */}
              <div className="col-span-4 flex flex-col min-h-[520px] bg-white">
...
# Replace the block
content = content[:db_tab_start_idx] + new_database_block + content[db_tab_end_line_idx:]
...
print("Database subtabs replaced successfully!")
